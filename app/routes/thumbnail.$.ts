...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  const percentage = parseInt(url.searchParams.get("perc") || "10");
  const width = parseInt(url.searchParams.get("w") || "0") || undefined
  let height = parseInt(url.searchParams.get("h") || "0") || undefined
  if (!width && !height) {
    height = 100
  }
  ...
  const thumbnail = await imageThumbnail({ uri: fullUrl }, {
    responseType: "buffer", percentage, width, height
  });
